...
def predict():
    try:
        ...
        # Use the UDPLag XGBoost model for UDPLag files.
        # This model matches UDPLag_scaler.pkl and UDPLag_feature_names.pkl better than the generic LSTM.
        if attack_type == "UDPLag":
            labels = model_xgb.predict(scaled_features)

            if hasattr(model_xgb, "predict_proba"):
                probabilities = model_xgb.predict_proba(scaled_features)
                confidences = np.max(probabilities, axis=1)
            else:
                confidences = np.ones(len(labels)) * 0.90
