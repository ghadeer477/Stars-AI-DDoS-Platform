...
def predict():
    try:
        ...
        # Use Deep Neural Network / MLP for UDPLag detection
        if attack_type == "UDPLag":
            mlp_features = mlp_scaler.transform(features)
            predictions = mlp_model.predict(mlp_features, verbose=0)
            ...
            # Binary output with shape (n, 1)
            elif predictions.shape[1] == 1:
                scores = predictions[:, 0]
                labels = (scores >= 0.5).astype(int)
                confidences = np.where(labels == 1, scores, 1 - scores)

            # Softmax output with two or more classes
            else:
            ...
            # Deep learning prediction for other attack types
        ...
        return jsonify({
            ...
            "model": "UDPLag MLP Deep Neural Network" if attack_type == "UDPLag" else attack_type,
...
def predict_live():
    ...
    try:
        ...
        sample_file_path = os.path.join(os.path.dirname(__file__), "UDPLag_real_attack_sample.csv")

        if not os.path.exists(sample_file_path):
            return jsonify({
                "success": False,
                "error": "UDPLag_real_attack_sample.csv not found in backend folder"
            })

        class LocalCSVFile:
            filename = "UDPLag_real_attack_sample.csv"

            def read(self):
                with open(sample_file_path, "rb") as f:
                    return f.read()

        features = extract_features_from_csv(LocalCSVFile())
        total_flows = int(features.shape[0])

        # Live detection using Deep Neural Network / MLP
        if attack_type == "UDPLag":
            mlp_features = mlp_scaler.transform(features)
            predictions = mlp_model.predict(mlp_features, verbose=0)
            predictions = np.array(predictions)
            ...
            labels = np.array(labels).astype(int)
            model_name = "UDPLag MLP Deep Neural Network"
        else:
            scaled_features = scaler.transform(features)
            predictions = dl_model.predict(scaled_features, verbose=0)
            predictions = np.array(predictions)
        ...
        if final_prediction == "ATTACK":
            alert = {
                "title": "Live DDoS Attack Detected",
                "source": "Live AI Monitor",
                "mitre": "T1498",
                "confidence": round(final_confidence, 2),
                "model": model_name
            }
        ...
        return jsonify({
            "success": True,
            "mode": "Live Deep Learning Monitor",
            "prediction": final_prediction,
            "result": final_prediction,
            "confidence": round(final_confidence, 2),
            "model": model_name,
